			...
			<n-modal v-model:show="showModal">
				...
				>
					...
					<div class="categoryModal">
						...
						<input type="text" name="" id="" v-model="categoryModel.name" class="inputCategory">

						<label>Kategoria nadrzędna</label>
						<div class="modelRow">
							<n-select :options="categoryOptions" v-model:value="categoryModel.parentCategoryId"/>
...
import { NButton, NModal, NCard, NDataTable, NSpin, NButtonGroup, NSelect } from "naive-ui";
...
const sort = (v,par) => {
	...
	categories.value = categories.value.sort(v.isAsc ? compareDSC : compareASC);
}
				  ...
				  h(
					  NButton,
					  {
						  onClick: ()=> {
							  ...
							  categoryOptions.value = categories.value.filter(f=>f.id!==row.id).map(f=>{
								  return {
									  label: f.name,
									  value: f.id
								  }
...
export default {
  ...
  components: {NDataTable, NButton, NCard, NModal, NSpin, TableSorter, NSelect}
};
</script>
