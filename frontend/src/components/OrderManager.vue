<template>
  <div class="wrapper">
    ...
    <n-spin :show="isLoading">
      ...
      <n-pagination :page-count="totalPages" v-model:page="orderState.page" />
    </n-spin>
  </div>
</template>
...
const columns = [
  ...
  },
  {
    ...
    render(row) {
      if (row.paymentType === "TRANSFER") {
        ...
        return h(
          NDropdown,
          { options: buff, onSelect: (v) => changePaymentStatus(v, row) },
          () => h(NButton, {}, () => mapped)
        );
      } else {
  ...
  },
  {
    ...
    render(row) {
      ...
      return h(
        NDropdown,
        { options: buff, onSelect: (v) => changeShipmentStatus(v, row) },
        () => h(NButton, {}, () => mapped)
      );
    },
  },
