<template>
	<div class="wrapper">
        ...
        <n-pagination style="margin: 10px; margin-left: auto;" v-if="!isLoading" v-model:page="userState.page" :page-count="totalPages"/>
	</div>
</template>

<script>
import { h , ref, watch, reactive} from "vue";
import { NButton, NModal, NCard, NDataTable, NBadge, NInput, NSpin, NPagination } from "naive-ui";
...
const getAllUsers = () => {
    ...
    axios({
        method: 'get',
        url: '/api/v1/user/admin?searchBy=' + searchInput.value + "&limit=10&page=" + (userState.page - 1) + "&order=" + userState.sort + "&sortBy=" + userState.sortBy,
    ...
    .then((response)=>{
        ...
        {
            console.log(exc);
            toast.error("Wystąpił błąd aplikacji", {timeout: 2000});
        }
...
watch(()=>{return {...userState}},(newValue, oldValue)=>{
...
export default {
  ...
  components: { NDataTable, NButton, NCard, NModal, NBadge, NInput, NSpin, TableSorterGroup, NPagination }
};
</script>
