    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        if(request.getServletPath().equals("/login") || request.getServletPath().equals("/register")){
            ...
            if(cookies == null){
                ...
                optional_access_token = Arrays.stream(cookies).filter(f -> f.getName().equals("access_token")).findFirst();
                optional_refresh_token = Arrays.stream(cookies).filter(f -> f.getName().equals("refresh_token")).findFirst();
            }
            ...
            if(optional_access_token.isEmpty() || optional_refresh_token.isEmpty())
            {
                if(authentication != null){
                    ...
                    String access_token = jwtService.generateJWT(user.getEmail(), (long) 1000 * 60 * 60 * 24).getToken();
                    String refresh_token = jwtService.generateJWT(user.getEmail(), 1000*60*60*24*7L).getToken();
            ...
            JWToken access_token = jwtService.verifyTokenAndGetSubject(optional_access_token.get().getValue());
            JWToken refresh_token = jwtService.verifyTokenAndGetSubject(optional_refresh_token.get().getValue());
            ...
            if(refresh_token.getIsExpired()){
                ...
                access_token = jwtService.generateJWT(refresh_token.getEmail(), (long) 1000 * 60 * 60 * 24);
                refresh_token = jwtService.generateJWT(refresh_token.getEmail(), 1000*60*60*24*7L);

                Cookie cookie_token = new Cookie("access_token", access_token.getToken());
                cookie_token.setHttpOnly(true);
                cookie_token.setMaxAge(Integer.MAX_VALUE);
                cookie_token.setDomain("localhost");
                cookie_token.setPath("/");

                Cookie cookie_refresh_token = new Cookie("refresh_token", refresh_token.getToken());
                cookie_refresh_token.setHttpOnly(true);
                cookie_refresh_token.setMaxAge(Integer.MAX_VALUE);
                cookie_refresh_token.setDomain("localhost");
                cookie_refresh_token.setPath("/");
