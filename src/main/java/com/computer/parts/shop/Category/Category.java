...
public class Category {
  ...
  @GeneratedValue(
    ...
    generator = "category_generator_sequence"
  )
  @SequenceGenerator(
    name = "category_generator_sequence",
  ...
  @Transient
  private CategoryDTO getCategoryDTOAsTree(
    Category category,
    Integer current,
    ...
    if (current.equals(maxTreeLevel)) return categoryDTO;

    List<CategoryDTO> categoryDTOList = new LinkedList<>();

    for (Category categoryBuff : category.categoryList) {
      CategoryDTO categoryDTOAsTree = categoryBuff.getCategoryDTOAsTree(
        categoryBuff,
        current,
        maxTreeLevel
      );
    ...
    categoryDTO.setCategoryList(categoryDTOList);
