...
public class PayPalController {
    ...
    @PostMapping
    public ResponseEntity<String> payment(@RequestBody PaymentRequest paymentRequest) throws PayPalRESTException {
        Payment payment = payPalService.createPayment(paymentRequest.getPrice(),
        ...
        Optional<Links> approval_url = payment.getLinks().stream().filter(f -> f.getRel().contains("approval_url")).findFirst();

        if(approval_url.isEmpty())
    ...
    public ResponseEntity<String> successPay(@RequestParam("paymentId") String paymentId, @RequestParam("PayerID") String PayerId) throws PayPalRESTException {
        ...
        return ResponseEntity.ok(payment.getState().equals("approved") ? "success" : "fail");
    }
